...
class SoldItem(models.Model):
    """
    Model representing a sold item in the system.

    This model stores information about an item sold by a user on Tartan Mart.

    Attributes:
        user (CustomUser): The user who sold the item.
        name (str): The name of the item.
        description (str): A detailed description of the item.
        price (Decimal): The price of the item.
        listed_date (DateTime): The date and time when the item was listed.
        ...
        delivery_or_pickup (str): The delivery method for the item (delivery or pickup).
        current_status (str): The current status of the item sold.
        category (str): The category of the item.
        delivery_time (int, optional): The estimated delivery time for the item.
        pickup_address (str, optional): The pickup address for the item (if pickup option is selected).
        image (ImageField, optional): An image of the item.
    ...
    current_status = models.CharField(max_length=20, choices=CURRENT_STATUS_CHOICES, default='sold')
    CATEGORY_CHOICES = (
        ('electronics', 'Electronics'),
        ('furniture', 'Furniture'),
        ('clothing', 'Clothing'),
    ...
    image_url = models.URLField(null=True, blank=True)
    def __str__(self):
        return f'Sold Item - {self.name}, Qty - {self.quantity}'
